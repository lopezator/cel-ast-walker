...
func WalkExpr(expression *exprpb.Expr) []interpreter.Instruction {
...
func (w *astWalker) walk(node *exprpb.Expr) []interpreter.Instruction {
	...
	return []interpreter.Instruction{}
}

func (w *astWalker) walkLiteral(node *exprpb.Expr) []interpreter.Instruction {
	...
	return []interpreter.Instruction{interpreter.NewLiteral(node.Id, value)}
}

func (w *astWalker) walkIdent(node *exprpb.Expr) []interpreter.Instruction {
	identName := node.GetIdentExpr().Name
	if _, found := w.scope.ref(identName); !found {
		ident := interpreter.NewIdent(node.Id, identName)
		w.scope.setRef(identName, node.Id)
		return []interpreter.Instruction{ident}
	}
	return []interpreter.Instruction{}
}

func (w *astWalker) walkSelect(node *exprpb.Expr) []interpreter.Instruction {
	sel := node.GetSelectExpr()
	operandID := w.getID(sel.Operand)
	return append(
		w.walk(sel.Operand),
		interpreter.NewSelect(node.Id, operandID, sel.Field))
}

func (w *astWalker) walkCall(node *exprpb.Expr) []interpreter.Instruction {
	...
	case operators.LogicalAnd, operators.LogicalOr:
		...
		return append(instructions, interpreter.NewCall(node.Id, call.Function, argIDs))
	...
	case operators.Conditional:
		...
		// 1: jump to <END> on undefined/error
		instructions = append(instructions,
			interpreter.NewJump(conditionID, len(trueVal)+len(falseVal)+3,
				jumpIfUnknownOrError(conditionID)))
		// 2: jump to <ELSE> on false.
		instructions = append(instructions,
			interpreter.NewJump(conditionID, len(trueVal)+2,
				jumpIfEqual(conditionID, types.False)))
		// 3: <IF> expr
		instructions = append(instructions, trueVal...)
		// 4: jump to <END>
		instructions = append(instructions,
			interpreter.NewJump(trueID, len(falseVal)+1, jumpAlways))
		// 5: <ELSE> expr
...
func (w *astWalker) walkList(node *exprpb.Expr) []interpreter.Instruction {
	listExpr := node.GetListExpr()
	var elementIDs []int64
	var elementSteps []interpreter.Instruction
	...
	return append(elementSteps, interpreter.NewList(node.Id, elementIDs))
}

func (w *astWalker) walkStruct(node *exprpb.Expr) []interpreter.Instruction {
	structExpr := node.GetStructExpr()
	keyValues := make(map[int64]int64)
	fieldValues := make(map[string]int64)
	var entrySteps []interpreter.Instruction
	...
	if len(structExpr.MessageName) == 0 {
		return append(entrySteps, interpreter.NewMap(node.Id, keyValues))
	}
	return append(entrySteps,
		interpreter.NewObject(node.Id, structExpr.MessageName, fieldValues))
}

func (w *astWalker) walkComprehension(node *exprpb.Expr) []interpreter.Instruction {
	...
	pushScopeStep := interpreter.NewPushScope(
		node.GetId(),
		map[string]int64{
	...
	// iter-init
	iterInitStep :=
		interpreter.NewCall(iteratorID, overloads.Iterator, []int64{w.getID(comprehensionRange)})
	...
	iterHasNextStep :=
		interpreter.NewCall(iterHasNextID, overloads.HasNext, []int64{iteratorID})
	// jump <END> if !it.hasNext()
	jumpIterEndStep :=
		interpreter.NewJump(iterHasNextID, loopInstructionCount-2, breakIfEnd(iterHasNextID))
	// eval x = it.next()
	// eval <cond>
	// jump <END> if condition false
	jumpConditionFalseStep :=
		interpreter.NewJump(loopID, loopInstructionCount-4, jumpIfEqual(loopID, types.False))

	// iter-next
	nextIterVarStep := interpreter.NewCall(iterNextID, overloads.Next, []int64{iteratorID})
	// assign the loop-step to the accu var
	accuUpdateStep := interpreter.NewMov(stepID, accuID)
	// jump <LOOP>
	jumpCondStep := interpreter.NewJump(stepID, -(loopInstructionCount - 2), jumpAlways)

	// <END> result
	resultSteps := w.walk(result)
	compResultUpdateStep := interpreter.NewMov(w.getID(result), w.getID(node))
	popScopeStep := interpreter.NewPopScope(w.getID(node))
	w.popScope()
...
func (w *astWalker) walkCallArgs(call *exprpb.Expr_Call) (
	argGroups [][]interpreter.Instruction, argGroupLens []int, argIDs []int64) {
	args := getArgs(call)
	argCount := len(args)
	argGroups = make([][]interpreter.Instruction, argCount)
...
func jumpIfUnknownOrError(exprID int64) func(interpreter.EvalState) bool {
	return func(s interpreter.EvalState) bool {
...
func breakIfEnd(conditionID int64) func(interpreter.EvalState) bool {
	return func(s interpreter.EvalState) bool {
...
func jumpIfEqual(exprID int64, value ref.Value) func(interpreter.EvalState) bool {
	return func(s interpreter.EvalState) bool {
		if val, found := s.Value(exprID); found {
			if types.IsBool(val) {
...
func jumpAlways(_ interpreter.EvalState) bool {
	return true
}
